/* ==============================================================================
   ...
   Project:     Assignment 6 - Functions
   Description: Refactor into functions
   =========================================================================== */
...
// Create character menu, adds the new character to state object
void createCharacterMenu(gameState *state, int displayWidth)
{
  std::vector<string> selections =
    {
...
void viewCharacterMenu(gameState *state, int displayWidth)
{
...
void editCharacterMenu(gameState *state, int displayWidth)
{
  int selection;
  int numSelections;
  do {
...
void deleteCharacterMenu(gameState *state, int displayWidth)
{
  int numCharacters;
  int selection;
  int numSelections;
  do {
    ...
    selection = display::getSelectionFromMenu("Delete Character",
                                              selections,
...
// menu to crud characters
void characterMenu(gameState *state, int displayWidth)
{
  ...
  do {
    ...
    charMenuSelection = display::getSelectionFromMenu("Character Menu",
                                                      *characterMenuOpts,
                                                      "Select an action");
    // empty character menu logic
    if (numCharacters == 0) {
      switch (charMenuSelection) {
        case 1: {
          createCharacterMenu(state, displayWidth);
          break;
        }
    ...
    // populated character menu logic
    } else {
      switch (charMenuSelection) {
        case 1: {
          createCharacterMenu(state, displayWidth);
        ...
        case 2: {
          viewCharacterMenu(state, displayWidth);
          break;
        }

        case 3: {
          editCharacterMenu(state, displayWidth);
          break;
        }

        case 4: {
          deleteCharacterMenu(state, displayWidth);
          break;
        }
...
void viewCharacterEquipmentMenu(character *selectedCharacter, int displayWidth)
{
  ...
  do {
    ...
    selection--; // correct selection for zero indexing
    if (selection < numSlots) {
      selectedCharacter->getEquipped(selection)->printGear(displayWidth);
    } else if (selection == numSlots) {
      for (int i=0; i < numSlots; i++) {
    ...
    selection++; // to make the while loop work
                 //
  } while (selection != numEquipmentMenuSelections);
}

// menu to choose a character with equipment to view said equipment
// TODO(): move this to character menu and replace with crud gear
void gearMenu(gameState *state, int displayWidth)
{
  // check if there are any characters to even choose from,
  // if not, notify user and return to previous menu
  int numCharacters = state->characters.size();
  if (numCharacters == 0) {
    display::printMessage(display::ERROR, "There are no characters to view.");
    return;
  }
  ...
  // we only care about the characters with gear equipped
  ...
  do {
    ...
    {
      viewCharacterEquipmentMenu(charactersWithEquipment[selection], displayWidth);
    }
    selection++; // to make the while loop work
...
// Game main menu
void mainMenu(gameState *state, int displayWidth)
{
  ...
  while (running)
  {
    ...
    {
      case 1: {
        characterMenu(state, displayWidth);
        break;
      }

      case 2: {
        gearMenu(state, displayWidth);
        break;
      }
...
void init(int argc, char *argv[], gameState *state, int displayWidth)
{
  // Create and display character from passed in filename if it is present
  if ( argc > 1) {
    // Get character info
    gameState::addCharacterToStateFromFile(state, argv[1]);
...
int main(int argc, char *argv[])
{
  ...
  init(argc, argv, &state, displayWidth);

  mainMenu(&state, displayWidth);
